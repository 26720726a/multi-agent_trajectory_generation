...
세 가지가 걸려 있다.
  ...
  1. agent 간 충돌은 `safety/project.py` 의 `_project_safe` 가 매 스텝 속도를
     줄여 *구조적으로* 막는다 (원본에서는 controller.py 안에 있었다).  "드물게 일어난다"가 아니라 "일어날 수 없다"가 설계다.  A2 에서 이
     함수를 벡터화할 때 이 성질이 조용히 사라지기 쉬우므로 테스트로 묶어 둔다.
  2. 검증은 `safety/validate.py` 가 한다.  플래너 내부를 전혀 보지 않고 실행된
