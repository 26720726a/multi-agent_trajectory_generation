...
from safety.geometry import point_rect_dist, seg_seg_min_dist  # noqa: F401
from .traj import Trajectory
from .world import Problem
from .worldmodel import Rollout

W_HARD = PLANNER.w_hard


@dataclass
class CostWeights:
    # 값은 원본 planner.py:52-60 그대로다.  출처만 config/planner.py 로 옮겼다.
    make: float = PLANNER.w_make    # team completion time -- the headline objective
    flow: float = PLANNER.w_flow    # break ties towards finishing individuals early
    wait: float = PLANNER.w_wait    # discourage yielding that buys nothing
    dist: float = PLANNER.w_dist    # discourage long detours
    clear: float = PLANNER.w_clear  # keep a comfortable margin, not just a legal one
    turn: float = PLANNER.w_turn    # discourage oscillation (a livelock precursor)
    dev: float = PLANNER.w_dev      # respect the reference route unless it costs time
    #: **controller 의 `vo_soft_margin`(0.22) 과 다른 값이다** — 원본에서 두
    #: 값이 같은 이름으로 공존해 왔다 (S0 §C-2).  통일은 S3 이후이며, 지금
    #: 맞추면 rollout 이 달라져 S2 게이트가 깨진다.
    soft_margin: float = PLANNER.cost_soft_margin  # m of separation we would like
...
class Planner:
    ...
    def evaluate(self, roll: Rollout, world_model) -> Cost:
        ...
        # NOTE(S2): `hit_horizon` 이면 미완 에이전트를 **한 명도** 세지 않는다.
        # 지평선에 걸린 rollout 은 어차피 전원 미완이라, 두 명이 남았든 다섯이
        # 남았든 하드 항이 똑같이 0 이 된다.  건드리지 않는다 — 원본 동작이고,
        # 검토는 reports/S2_migration.md §6 "S3 이후 후보".
