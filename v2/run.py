...
#: 오히려 느려진다.  그래서 numpy 가 딸려 오는 아래 planning import 보다 **위에서**
...
from config import (CONTROLLER, PHYSICS, PLANNER, PHYS_FP,
                    physics_fingerprint)
import safety.validate as V
from planning.coordination import (critical_path_bound, plan_coordination,
                                   plan_prioritized, plan_sequential)
from planning.execute import WMConfig, run_wm_planner
from planning.paths import build_prior
...
FIELDS = [
    ...
    # -- S3-A 에서 추가.  wm_planner 행에만 값이 있다. --
    #: 실행된 궤적에서 |dv|/dt 가 a_max 를 넘은 (에이전트 x 스텝) 표본 수.
    #: `_project_safe` 는 일부러 a_max 로 제한하지 않으므로 (제한하면 충돌
    #: 방지가 깨진다) 그 초과분이 여기 잡힌다.  숨기지 않고 잰다 (S3 §A-2).
    "n_amax_violations",
    #: 위반의 원인 분해 (셋의 합 = n_amax_violations).  원인이 다르면 대응도
    #: 다르다 — 자세한 것은 `planning/control.py::new_stats`.
    #:   stop    HOI dwell 진입 / goal 도착의 순간 정지 (과업 정의)
    #:   block   후보 65개가 전부 기하 필터에 걸려 강제로 선 것
    #:   snap    W/G 정확 착지 대입
    #:   project `_project_safe` 가 넘긴 것 <- S3 §A-2 가 재라고 한 숫자
    "n_amax_viol_stop", "n_amax_viol_block", "n_amax_viol_snap",
    "n_amax_viol_project",
    #: 안전 투영이 명령을 실제로 바꾼 표본 수.  "안전이 계획을 얼마나 자주
    #: 덮어쓰는가".
    "n_project_active",
    #: 후보가 전멸해 강제로 선 표본 수.  a_max 가 후보를 이전 속도 주변으로
    #: 모으므로 이 값이 늘 수 있다 — "갈 곳이 없어졌는가".
    "n_blocked",
    #: S3-B2 다단계 하드 필터가 실제로 `free_steps` 를 부른 횟수. 후보가 첫
    #: 세그먼트에서 막히면 뒤는 조기 종료하므로, 65개 x 4 같은 상한보다 실측이
    #: 비용 판단에 더 알맞다.
    "n_free_steps_calls",
    # -- S3-S 에서 추가: 침범 판정의 **단일 출처**. --
    #
    # 지금까지 침범을 아는 경로가 셋이었고(`status`, `valid`, 두 clearance 열)
    # 셋이 서로 다른 답을 냈다.  `status` 는 완주 실패면 덮어써져 항상
    # `invalid` 가 0 이었고, 그 위에서 "충돌 0건" 게이트를 네 번 통과시켰다
    # (S3-B3 §7-1).
    #
    # 이제 `safety/validate.py` 의 판정을 **그대로** 옮겨 적는다.  아래 두 열이
    # 유일한 판정 근거이고, `status` 와 독립적으로 살아남는다.
    "n_agent_violations", "n_obstacle_violations",
    #: 같은 검증기가 낸 최소 여유.  판정이 아니라 **얼마나 아슬아슬했는지** 를
    #: 본다.  기존 `agent_clearance` / `obstacle_clearance` 와 값은 같고
    #: 소수 6자리로 적는다 (기존 열은 4자리라 경계 근처에서 반올림이 섞인다).
    "min_agent_clearance", "min_obstacle_clearance",
    #: 처음 침범한 타임스텝.  없으면 -1.  어디를 들여다볼지 가리킨다.
    "first_violation_t",
    # -- S3-C 에서 추가. --
    #: **모든 재계획**에서 채점한 후보 수와, 그중 `Cost.stalled` 가 True 였던 수.
    #: horizon_steps <= stall_window(45) 면 구조적으로 0 이다 — h=4.0 이 그렇다.
    #: 지평선을 올릴 때 정체 판정이 살아나는 시점이 성능 변화와 겹치는지를 본다.
    "n_evals", "n_stalled_evals",
    # -- S5-1: A*/문제 생성/검증을 뺀 재계획 본체의 인스턴스별 지연. --
    "n_replans", "n_replans_over_1s", "replan_ms_median",
    "replan_ms_p90", "replan_ms_max",
    #: 관측된 최대 |dv|/dt.  `a_max=inf` 인 기준선에서도 의미가 있다 —
    #: "지금 컨트롤러가 실제로 얼마나 급하게 움직이는가" 에 답한다.
    "max_accel",
    # -- S3 에서 추가 (S0 §G-3 위험 3). --
    #: 이 행이 어느 물리에서 나왔는지.  uid 는 문제 정의만 해싱하므로 v_max 나
    #: a_max 를 바꿔도 그대로다 — 짝지은 비교에는 그게 필요하지만, 서로 다른
    #: 물리로 돈 행이 한 CSV 에 섞이면 안 된다.  `scan_existing` 이 재개할 때
    #: 이 열을 보고 다르면 SchemaMismatch 와 같은 급으로 거부한다.
    "phys_fp",
]
...
def effective_fingerprint(wm_cfg: Optional[Dict] = None) -> str:
    """이 배치가 **실제로** 돌린 물리의 지문.

    `config.PHYS_FP` 를 그대로 쓰면 안 된다.  bench config 의 `wm_config` 가
    `horizon_s` / `replan_s` 를 덮어쓰기 때문이다 — 지평선 스윕(S3-C)은 정확히
    그것을 바꾸며 돌리는 실험이고, 그때 지문이 전부 같게 나오면 이 열은
    "물리가 같은가" 라는 질문에 거짓말을 하게 된다.

    `stall_window` 는 `WMConfig` 에 없다 (worldmodel 의 인자 기본값이라 config
    가 유일한 출처다).  그래서 덮어쓸 수 없고, 여기서도 config 값을 쓴다.
    """
    over = {k: wm_cfg[k] for k in ("horizon_s", "replan_s")
            if wm_cfg and k in wm_cfg}
    planner = dataclasses.replace(PLANNER, **over) if over else PLANNER
    return physics_fingerprint(PHYSICS, planner, CONTROLLER)
...
def _blank(inst: Instance, method: str, lb: Optional[float], commit: str,
           planner_seed: int, timeout_s: Optional[float],
           phys_fp: str = PHYS_FP) -> Dict[str, object]:
    row = {k: "" for k in FIELDS}
    row.update(inst.row())
    row.update(method=method, status="ok", git_commit=commit,
               phys_fp=phys_fp, planner_seed=planner_seed,
...
def _fill(row: Dict[str, object], problem, sol, rep, runtime: float,
    ...
    if rep is not None:
        ...
        # S3-S: 검증기의 판정을 그대로 옮긴다.  아래 `status` 덮어쓰기와
        # **무관하게** 살아남는 것이 요점이다.
        row.update(n_agent_violations=int(rep.n_agent_violations),
                   n_obstacle_violations=int(rep.n_obstacle_violations),
                   min_agent_clearance=round(float(rep.agent_clearance), 6),
                   min_obstacle_clearance=round(float(rep.obstacle_clearance), 6),
                   first_violation_t=int(rep.first_violation_t))
        if not rep.valid:
            row["status"] = "invalid"
    if lb:
...
def run_instance(inst: Instance, wm_cfg: Dict, methods: List[str], commit: str,
    ...
    fp = effective_fingerprint(wm_cfg)
    ...
    if problem is None:
        ...
        if planner_seed != deterministic_seed:
            return []
        r = _blank(inst, "-", None, commit, planner_seed, timeout_s, fp)
    ...
    if "lower_bound" in todo:
        r = _blank(inst, "lower_bound", lb, commit, planner_seed, timeout_s, fp)
        r.update(team_time=round(lb, 3), ratio_to_bound=1.0, runtime_s=0.0)
        rows.append(r)

    if "sequential" in todo:
        r = _blank(inst, "sequential", lb, commit, planner_seed, timeout_s, fp)
    ...
    if "prioritized" in todo:
        ...
        # 더 가깝다.
        r = _blank(inst, "prioritized", lb, commit, planner_seed, timeout_s, fp)
        t0 = time.perf_counter()
        try:
    ...
    if "coordination_astar" in todo:
        r = _blank(inst, "coordination_astar", lb, commit, planner_seed, timeout_s, fp)
    ...
    if "wm_planner" in todo:
        r = _blank(inst, "wm_planner", lb, commit, planner_seed, timeout_s, fp)
        t0 = time.perf_counter()
        try:
            ...
            r["n_evals"] = res.n_evals
            r["n_stalled_evals"] = res.n_stalled_evals
            ts = res.replan_ms
            r["n_replans"] = len(ts)
            r["n_replans_over_1s"] = sum(t > 1000.0 for t in ts)
            if ts:
                r["replan_ms_median"] = round(float(np.median(ts)), 3)
                r["replan_ms_p90"] = round(float(np.percentile(ts, 90)), 3)
                r["replan_ms_max"] = round(float(max(ts)), 3)
            r.update(wm_mode_stats(res))
            acc = res.traj.extra.get("accel") or {}
            for col in ("n_amax_violations", "n_amax_viol_stop",
                        "n_amax_viol_block", "n_amax_viol_snap",
                        "n_amax_viol_project", "n_project_active", "n_blocked",
                        "n_free_steps_calls"):
                r[col] = int(acc.get(col, 0))
            r["max_accel"] = round(float(acc.get("max_accel", 0.0)), 4)
...
def run_instance_guarded(inst: Instance, wm_cfg: Dict, methods: List[str],
    ...
    rows = []
    for m in todo:
        r = _blank(inst, m, None, commit, planner_seed, timeout_s, fp)
...
class PhysicsMismatch(RuntimeError):
    """기존 CSV 의 행이 **다른 물리**로 돌아간 것이다.

    `SchemaMismatch` 와 같은 급으로 다룬다.  열 이름이 맞아도 v_max 가 다른
    행과 섞이면 그 CSV 로는 아무것도 결론지을 수 없고, 더 나쁜 것은 uid 가 같아
    **짝지은 비교가 조용히 성립해 버린다**는 점이다 (S0 §G-3 위험 3).
    """
...
def scan_existing(path: str, fields: Sequence[str] = FIELDS,
    ...
    `phys_fp` 열이 지금 설정의 지문과 다른 행을 만나면 `PhysicsMismatch` 를
    던진다 — 잘린 행처럼 조용히 버리지 않는다.  버리면 "이어서 돌리면 되겠지"
    가 되는데, 실제로는 **같은 uid 에 두 물리의 행이 공존**하게 되어 나중에
    짝지은 비교가 아무 경고 없이 틀린 답을 낸다.
    """
    want_fp = PHYS_FP if phys_fp is None else phys_fp
    ...
    for i, ln in enumerate(lines[1:], start=1):
        ...
        got_fp = rec.get("phys_fp", "")
        if got_fp != want_fp:
            raise PhysicsMismatch(
                f"  {i}번째 행 (uid={rec.get('uid')!r}, "
                f"method={rec.get('method')!r}) 의 phys_fp 가 {got_fp!r} 인데 "
                f"지금 설정은 {want_fp!r} 다.\n"
                f"  이 CSV 는 다른 물리(v_max / a_max / horizon / controller 튜너블)"
                f"에서 나온 것이다.")
        done.add(_key(rec))
        good_end += len(ln)
        n_good += 1
...
def main() -> None:
    ...
    if resuming:
        try:
            done, n_good, n_dropped, good_end = scan_existing(
                out, phys_fp=effective_fingerprint(wm_cfg))
            ...
            raise SystemExit(2)
        except PhysicsMismatch as exc:
            print(f"\n기존 CSV 가 **다른 물리**로 돌아간 것이다: {out}\n{exc}\n\n"
                  f"물리가 섞인 CSV 는 uid 가 같아서 짝지은 비교가 조용히 성립해 "
                  f"버린다 — 그래서 스키마 불일치와 같은 급으로 멈춘다.  다른 "
                  f"--out 을 쓰거나, 기존 결과를 버려도 된다면 --fresh 를 붙인다.",
                  file=sys.stderr)
            raise SystemExit(2)
        if n_dropped:
    ...
    print(f"{cfg.get('name', 'run')}: 격자 {n_grid}, 인스턴스 {len(insts)} x "
          ...
          f"  commit {commit},  물리 지문 {effective_fingerprint(wm_cfg)}\n"
          f"-> {out}\n", flush=True)
