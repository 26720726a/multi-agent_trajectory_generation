...
class AgentSpec:
    ...
    waypoint: Point
    goal: Point
    # 기본값은 config/physics.py 가 유일한 출처다 (S0 §C-2: v_max 가 여기와
    # scripts/build_dataset.py 에 서로 다른 값으로 두 번 적혀 있었다).
    # 값 자체는 원본과 같다: 1.0 / 0.30 / 1.20 / 0.55.
    dwell: float = PHYSICS.dwell_s       # seconds spent at the waypoint (the "HOI")
    radius: float = PHYSICS.robot_radius       # physical footprint
    v_max: float = PHYSICS.v_max         # m/s
    interact_radius: float = PHYSICS.interact_radius  # visual/semantic radius of the HOI region
    color: str = PALETTE[0]
...
class Problem:
    ...
    safety: float = PHYSICS.safety_margin  # extra clearance between agent footprints (m)
    dt: float = PHYSICS.dt        # simulation timestep (s)
    description: str = ""
...
# --------------------------------------------------------------------------- #
def random_problem(seed: int, n_agents: int = 2, size: float = PHYSICS.map_size,
    ...
    *strength* of the coupling, where `coupled_waypoints` is only its *rate*.
    ...
    # 원본 world.py:265 는 여기서 radius 를 다시 선언했다 — AgentSpec.radius 와
    # 어긋날 수 있는 자리였다 (S0 §C-2).  이제 둘 다 config 를 본다.  값은 동일.
    radius, margin = PHYSICS.robot_radius, PHYSICS.gen_obstacle_margin
