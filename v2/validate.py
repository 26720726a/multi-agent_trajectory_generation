...
# `Problem`(planning/world.py) 과 `Solution`(planning/coordination.py) 은 **타입
# 표기로만** 쓰인다.  safety/ 는 planning/ 을 import 하지 않는다는 계층 규칙
# (tests/test_layering.py (a)) 때문에 실제 import 를 두지 않고, `from __future__
# import annotations` 아래에서 문자열로만 남는 전방 참조를 쓴다.  런타임 동작은
# 원본과 완전히 같다 — 원본에서도 이 두 이름은 어노테이션 외에 쓰이지 않았다.
...
class Report:
    ...
    #: 처음으로 침범이 일어난 타임스텝.  침범이 없으면 `-1`.
    #:
    #: **계측이지 판정이 아니다** — 위반 여부와 개수는 위 세 필드가 정하고,
    #: 이 값은 "어디를 들여다볼 것인가" 를 가리킬 뿐이다 (S3-S).  그래서
    #: 이것을 더해도 검증 결과는 한 건도 바뀌지 않는다.
    first_violation_t: int = -1
...
def _note_first(rep: Report, t: int) -> None:
    """가장 이른 침범 시점을 남긴다 (S3-S 계측).  판정에는 관여하지 않는다."""
    if t < 0:
        return
    if rep.first_violation_t < 0 or t < rep.first_violation_t:
        rep.first_violation_t = t
...
def validate(problem: "Problem", sol: "Solution",
    ...
    for i, a in enumerate(problem.agents):
        ...
        if clr < a.radius - 1e-3:
            ...
            # 4배 조밀화한 인덱스를 원래 스텝으로 되돌린다 (S3-S 계측).
            per = np.full(len(pts), np.inf)
            for r in problem.world.obstacles:
                per = np.minimum(per, point_rect_dist(pts, r))
            per = np.minimum.reduce([
                per, pts[:, 0], pts[:, 1],
                problem.world.width - pts[:, 0],
                problem.world.height - pts[:, 1]])
            bad = np.flatnonzero(per < a.radius - 1e-3)
            if len(bad):
                _note_first(rep, int(bad[0]) // 4)
    ...
    for i in range(n):
        for j in range(i + 1, n):
            ...
            if worst < need - 1e-3:
                ...
                _note_first(rep, first_t if first_t >= 0 else T)
                rep.errors.append(
