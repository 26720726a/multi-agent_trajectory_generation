...
def _row(uid: str, method: str, ps: int, **kw) -> dict:
    # S3: `phys_fp` 를 채워 둔다.  `scan_existing` 이 이 열을 지금 설정의 지문과
    # 대조하므로, 비워 두면 "다른 물리로 돈 CSV" 로 거부당한다 — 그 거부 동작
    # 자체는 tests/test_physics_fingerprint.py 가 확인한다.
    r = {k: "" for k in FIELDS}
