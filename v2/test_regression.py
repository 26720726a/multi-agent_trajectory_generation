...
기준선을 의도적으로 갱신할 때:
    ...
    python scripts/check_wm.py                    # 눈으로 확인
    python tests/test_regression.py --update      # results/baseline_v0.json 갱신

`results/baseline_v0.json` 은 원본 저장소의 것을 **그대로** 가져왔다.  S2 의
게이트가 "원본과 같은 결과"이므로 기준선도 원본의 것이어야 한다.
...
def test_lower_bound_is_never_beaten():
    ...
    from planning.coordination import critical_path_bound
