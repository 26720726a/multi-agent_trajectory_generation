...
def test_a_half_written_last_row_is_dropped_and_reported(tmp_path):
    ...
    whole = p.read_bytes()
    # 마지막 **줄의 절반**을 자른다.  고정 바이트 수(예전에는 20)를 쓰면 FIELDS
    # 에 열이 늘어날 때 끝이 빈칸+콤마라 잘라도 유효한 행으로 파싱된다 —
    # S3-S 에서 판정 열 5개를 더하자 실제로 그렇게 됐다.
    head, _, last = whole.rpartition(b"\r\n" if b"\r\n" in whole else b"\n")
    cut = last if last else whole.split(b"\n")[-2]
    p.write_bytes(whole[:len(whole) - len(cut) // 2])
